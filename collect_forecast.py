...
def fetch_forecast(grid):
    ...
    for d in daily:
        ...
        # Resolve PoP by taking the maximum across both sources:
        #   - gridpoint probabilityOfPrecipitation (daily_max of hourly values)
        #   - period-level probabilityOfPrecipitation (embedded in the forecast period)
        # The two NWS products can disagree even within the same updateTime.
        # Taking the max preserves any non-zero signal from either source.
        gridpoint_pop = round(pop[dt]) if dt in pop else None
...
def ensure_output_file():
    ...
    # File exists — check if the first line is the expected header.
    # If not (e.g. old data written without a header), prepend it now.
    with open(OUTPUT_FILE, "r", newline="") as f:
        first_line = f.readline().strip()

    expected_header = ",".join(CSV_COLUMNS)
    if first_line != expected_header:
        print(f"  Header missing — prepending to existing data…")
