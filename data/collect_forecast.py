...
def fetch_forecast(grid):
    ...
    # ── DIAGNOSTIC — remove after confirming PoP values are correct ──────────
    print("  [DIAG] Period-level PoP per date:")
    for d in daily:
        print(f"    {d['date']}  _period_pop={d.get('_period_pop')}")
    print("  [DIAG] Gridpoint PoP dict:")
    for k, v in sorted(pop.items()):
        print(f"    {k}  gridpoint_pop={v}")
    # ── END DIAGNOSTIC ────────────────────────────────────────────────────────
